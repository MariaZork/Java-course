...
вспомогательные классы и интерфейсы) и отправить их код в форму. Все классы должны быть публичными и статическими
...
public class Task_33 {
    ...
    public static class MailService<T> implements Consumer<Sendable<T>> {
        private Map<String, List<T>> box = new HashMap<>() {
            ...
            public List<T> get(Object key) {
                ...
                try {
                    if (temp == null) {
                        return super.getOrDefault((String) key, new LinkedList<>());
                    }
        ...
        public void accept(Sendable<T> message) {
            if (box.containsKey(message.getTo())) {
                List<T> values;
                values = box.get(message.getTo());
                values.add(message.getContent());
                box.put(message.getTo(), values);
            }
            else {
                List<T> values;
    ...
    public static void test() {
        String randomFrom = "Austin Powers"; // Некоторая случайная строка. Можете выбрать ее самостоятельно.
        String randomTo = "James Bond";  // Некоторая случайная строка. Можете выбрать ее самостоятельно.
        int randomSalary = 100;  // Некоторое случайное целое положительное число. Можете выбрать его самостоятельно.
